...
gen_alnum_string <- function(min_len = 1, max_len = 10) {
  gen.and_then(gen.element(min_len:max_len), function(len) {
    gen.map(
      ...
      gen.c(gen.element(c(letters, LETTERS, as.character(0:9))), from = len, to = len)
    )
  })
}
...
gen_col_name <- function() {
  gen.and_then(gen.element(c(letters, LETTERS)), function(first) {
    gen.map(
      function(rest) paste0(first, paste0(rest, collapse = "")),
      gen.c(gen.element(c(letters, LETTERS, as.character(0:9))), from = 1, to = 6)
    )
  })
}
...
test_that("Property 1: SaveData/LoadData round trip preserves data", {
  ...
  forall(gen_data_frame(), function(df) {
    ...
    function(meta, snapshot_id, artifact_name) {
      ...
      for (p in csv_paths) {
        expect_true(
          ...
          info = sprintf("Path '%s' should contain snapshot_id '%s'", p, snapshot_id)
        )
        expect_true(
          grepl(meta$ID, p, fixed = TRUE),
          info = sprintf("Path '%s' should contain workflow ID '%s'", p, meta$ID)
        )
      }
    },
    tests = 100
...
test_that("Property 3: Failed steps recorded and subsequent workflows execute", {
  ...
  forall(gen_pipeline, function(pipeline_spec) {
    ...
    for (i in seq_len(n_wf)) {
      ...
      if (i == fail_idx) {
        steps <- list(
          record_step_status(
            paste0("pkg::step_", i, "_1"), paste0("out_", i, "_1"), "completed"
          ),
          record_step_status(
            paste0("pkg::step_", i, "_2"), paste0("out_", i, "_2"), "failed",
            error = sprintf("Error in step %d: simulated failure", i)
          )
        )
        workflow_results[[wf_name]] <- list(
          workflow_id = wf_id, workflow_type = "Metric",
...
test_that("Property 4: Data config parsing maps all domains to non-empty paths", {
      ...
      function(raw_domains) {
        ...
        gen.map(
          function(path_parts) {
            ...
            lines <- vapply(seq_along(domains), function(i) {
  ...
  forall(gen_data_config, function(config_spec) {
    ...
    for (domain in config_spec$domains) {
      expect_true(
        ...
        info = sprintf("lConfig$data_config should contain domain '%s'", domain)
      )
    }
  }, tests = 100)
})
...
test_that("Property 17: Snapshot listing returns correct count, uniqueness, and metadata", {
  ...
  forall(gen_snapshot_sequence, function(spec) {
    n <- spec$count
    ...
    expect_true(all(c("snapshot_id", "created_at", "input_data_version",
                       "package_snapshot") %in% names(result)))
...
test_that("Property 18: LoadData returns correct precedence across snapshots", {
  gen_inheritance_scenario <- gen.and_then(gen.element(2:5), function(n_snapshots) {
  ...
  local_mocked_bindings(
    gh_get_content = function(repo, path, branch, token) {
      if (grepl("snapshots\\.json", path)) {
        content <- jsonlite::toJSON(mock_store$snapshots_json, auto_unbox = TRUE)
  ...
  forall(gen_inheritance_scenario, function(scenario) {
    ...
    if (length(domains) > 1) {
      for (d in domains[-1]) {
        ...
        expect_equal(
          ...
          info = sprintf("Domain '%s' should be inherited from previous snapshot", d)
        )
      }
    }
  }, tests = 100)
